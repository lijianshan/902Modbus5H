...
public class dmsg {
	...
	private static int mPort(String url) {
		...
		for(int i=0; i<msg_port.length; i++) {
			if (url.indexOf(msg_port[i].url) == 0)
	...
	public static Boolean start(String url) {
		...
		try {
			...
			ds.bind(new InetSocketAddress("127.0.0.1", port));*/
	...
	private static Boolean is_utf8(byte [] data, int length) {
		...
		for(int i=0; i<length; i++) {
			...
			if ((data[i+0] & 0xE0) == 0xC0 && i+1<=data.length) { //˫�ֽڸ�ʽ
			...
			} else if ((data[i+0] & 0xF0) == 0xE0 && i+2<=data.length) { //���ֽڸ�ʽ
				if (((data[i+1] & 0xC0) == 0x80) && ((data[i+2] & 0xC0) == 0x80)) {
					utf8_n++;
					i += 2;
					continue;
				}
            } else if ((data[i+0] & 0xF8) == 0xF0 && i+3<=data.length) { //���ֽڸ�ʽ
